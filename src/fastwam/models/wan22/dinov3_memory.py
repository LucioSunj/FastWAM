...
class DinoV3AssetSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if model_name != PINNED_DINOV3_MODEL_NAME:
            raise ValueError("P8 requires the pinned dinov3_vits16 architecture.")
...
def _local_dinov3_import(source_root: Path):
    """Import the pinned backbone module without optional hub tasks."""
...
def _load_tensor_state(path: Path) -> dict[str, torch.Tensor]:
    """Load a local PyTorch or safetensors flat tensor state."""
...
class FrozenDinoV3Encoder(nn.Module):
    ...
    def encode(self, camera_batch: PreparedCameraBatch) -> NativePatchMemory:
        ...
        # The trainable refiner saves native memory for backward.  A detached
        # inference tensor is still illegal there, so materialize normal tensor
        # semantics only after leaving the frozen encoder's inference scope.
