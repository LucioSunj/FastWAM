...
class FastWAMProcessor(BaseProcessor):
    def __init__(
        ...
        self.num_image_steps = (
            num_obs_steps if num_image_steps is None else int(num_image_steps)
        )
        if self.num_image_steps < 1:
            raise ValueError("num_image_steps must be positive.")
